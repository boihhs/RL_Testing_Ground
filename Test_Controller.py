...
with viewer.launch_passive(mj_model, mj_data) as v:
    while v.is_running():
        ...
        torque_tired = jnp.linalg.norm(current_ctrl/jnp.array(cfg["PPO"]["torque_limit"])) * -1e-2
        ...
        # action_rate = 
        joint_pos_limit = jnp.sum(jnp.where(joint_pos > jnp.array(cfg["PPO"]["joint_q_max"]), 1, 0) + jnp.where(joint_pos < jnp.array(cfg["PPO"]["joint_q_min"]), 1, 0)) * -1.
        # collison = 
